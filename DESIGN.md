...
### Contact (I&I addition)
The contact form sits at the foot of every page rather than on a page of its own, so the way in is never more than one scroll away. Calls to action anywhere on the site point at it. There is no phone number.

### Opening flicker (I&I addition)
The home page opens with a full-screen white overlay running through all 24 ways of writing "i and i" — four marks (i, I, and two pairs of eyes) against six conjunctions that genuinely mean "and" (the ampersand, the plus, the Tironian et, the logical and, the doubled ampersand, and the Latin word). It opens on i&i and always lands back on i&i, holding it for a beat before it goes. It fades out on the first sign of intent: a mouse move, a scroll, a tap or a keystroke. A short grace period stops an already-moving pointer skipping it, a safety timer dismisses it regardless, and a `noscript` rule removes it when JavaScript is unavailable. It plays once per visit, on the home page only, and holds a single frame under `prefers-reduced-motion`. The mark drifts through the same palette as the moving gradient, as colour rather than a clipped gradient so the eyes stay eyes.
...
On the home page the placements screen hangs seven works along a wall that turns away from the viewer, drawn with WebGL. Four are in view on a wide screen and two and a half on a narrow one; that count is the fixed thing, and the spacing between works follows from it and the size of the screen. Each work fills the width of its slot, so proportions vary in height rather than width. Its section is twice the viewport tall and its content sticks to the top, so the scrolling between walks the camera down the wall; the page itself is never hijacked. Every work keeps its own proportions, read from the file at build time, so nothing is cropped. Each casts a soft shadow on the wall, down and to the right, so it reads as hanging off the surface rather than printed on it. That is the one shadow in the whole system, and it belongs to the scene rather than to the interface. The caption beneath names whichever work is in front of you, and the pointer tips the camera very slightly.
...
On the founder quote, hovering a word brings up an open-access work from the Cleveland Museum of Art whose title carries that word, behind the type at 32% opacity, with a credit line at the foot. The match is strict: the word has to be in the title. It is stemmed first, so "collecting" reaches Collected and "decades" would reach Decade, and where nothing matches nothing is shown. The credit picks the shared word out in Dark Spruce, and both the credit and the hovered word link to the work at the museum. Words shorter than four letters and a short list of function words are skipped. The collection's API sends no CORS headers, so requests pass through the site's own `/api/artwork` route, which caches each word for a day. A failed lookup shows nothing; it never interrupts the page.
